...
package com.exactpro.th2.processor.core.message.protobuf.controller.state
...
internal class CradleMessageGroupState(
    ...
    fun plus(func: StateUpdater<MessageGroupBatch, MessageGroup>.() -> Unit): Boolean {
        val temporaryState = mutableMapOf<StateKey, Long>()
        object : StateUpdater<MessageGroupBatch, MessageGroup> {
            @Suppress("PARAMETER_NAME_CHANGED_ON_OVERRIDE")
            override fun updateState(batch: MessageGroupBatch, group: MessageGroup) {
                temporaryState.compute(group.check().toStateKey()) { _, current -> (current ?: 0L) + 1 }
            }
        }.func()
